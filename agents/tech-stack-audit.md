...
**Task**: Flesh out tech stack in README, verify docs are up to date, fix ngrok HTTPS issue  
...
1. **README.md — Complete rewrite** with detailed tech stack tables covering:
   ...
   - Backend (Node.js http server, `ws` WebSocket, ngrok)
...
2. **scripts/start-public.sh — ngrok HTTPS fix**
   - Changed `ngrok http "$PORT"` → `ngrok http --scheme=https "$PORT"`
   - User confirmed the issue was ngrok not specifying HTTPS, which prevented secure WebSocket (`wss://`) pairing
